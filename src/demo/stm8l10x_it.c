...
extern bool go_sleep,count_dir;
extern uint16_t Counter,slp;
...
INTERRUPT_HANDLER(TIM2_UPD_OVF_TRG_BRK_IRQHandler, 19)
{
    /* In order to detect unexpected events during development,
       it is recommended to set a breakpoint on the following instruction.
    */
	TIM2->IER &= ~(TIM_IER_UIE); // Interrupt disable
  if ( TIM2->SR1 & TIM_SR1_UIF ) { 
			if (!count_dir) {
				if (TIM2->CR1 & TIM_CR1_DIR) {Counter--;TIM2->CNTRL=4;}
				else 												 {Counter++;TIM2->CNTRL=0;}
			}
			else {// count_dir
				if (TIM2->CR1 & TIM_CR1_DIR) {Counter++;TIM2->CNTRL=4;}
				else 												 {Counter--;TIM2->CNTRL=0;}
	...
	slp=30000;
	TIM2->IER |= TIM_IER_UIE; //Interrupt enable	 
